...
function AdminLogin({ onLogin, role = 'admin' }) {
  ...
  const title = isStore ? 'Magasin' : isDepot ? 'Dépôt' : 'Administration';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { token, magasinId, magasinName } = await api.login(hashHex, role);
      sessionStorage.setItem('auth_token', token);
      sessionStorage.setItem('auth_role', role);
      onLogin(role, magasinId, magasinName);
    } catch (err) {
